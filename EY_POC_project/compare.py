...
#read the xlsx file
file_name = "../EY_POC_project/data\\transcripts_july141.csv"
...
for index, data in excel.iterrows():
    original.append(data["Original Transcription"].translate({ord(c): " " for c in "'!@#$%^&*()’[]{};:,./<>?\|`~-=_+"}).lower())
    regular.append(data["Google regular Transcription"].translate({ord(c): " " for c in "'!@#$%^&*()’[]{};:,./<>?\|`~-=_+"}).lower())
    Enhanced.append(data["Google Enhanced Transcriptions"].translate({ord(c): " " for c in "'!@#$%^&*()’[]{};:,./<>?\|`~-=_+"}).lower())
    Azure.append(data["Azure Transcription"].translate({ord(c): " " for c in "'!@#$%^&*()’[]{};:,./<>?\|`~-=_+"}).lower())
    AWS.append(data["AWS Transcriptions"].translate({ord(c): " " for c in "'!@#$%^&*()’[]{};:,./<>?\|`~-=_+"}).lower())
...
#
excel["Google_regular_word_accuracy"] = Google_regular_word
excel["Google_Enhanced_word_accuracy"] = Google_Enhanced_word
excel["Azure_word_accuracy"] = Azure_word
excel["AWS_word_accuracy"] = AWS_word

df = pd.DataFrame(list(zip(original, regular, Enhanced, Azure, AWS,
                           Google_regular_letter, Google_Enhanced_letter, Azure_letter, AWS_letter,
                           Google_regular_word, Google_Enhanced_word, Azure_word, AWS_word)),
                    columns =['original', 'regular', 'Enhanced', 'Azure', 'AWS', 'Google_regular_letter',
                              'Google_Enhanced_letter', 'Azure_letter', 'AWS_letter', 'Google_regular_word',
                              'Google_Enhanced_word', 'Azure_word', 'AWS_word'])

# writing output in csv format
excel.to_csv('output\\transcript_accuracy_july20.csv', index=False)
df.to_csv('output\\results_of_transcript_accuracy.csv', index=False, encoding="utf-8")
